...
contract Game{
    ...
    function update() public {
        ...
        tokenIdTBS = _CoinPool.tokenIdTBS();
    ...
    function encode(address player, uint256 trxvalue, uint256 tbsvalue, uint256 number,uint32 betType)internal pure returns(bytes32){
    ...
    function decode(bytes32 _en) internal pure returns(address player, uint256 trxvalue, uint256 tbsvalue, uint256 number,uint32 betType){
        uint256 en = uint256(_en);
        player = address(en >> (12*8));
        trxvalue = tbsvalue = 0;
        uint256 value = (en >> (8*8));
        if ((value & (1<<31)) > 0)
            tbsvalue = (value & ((1<<31) - 1))*1e6;
        else
    ...
    function tibet(uint32 betType) internal gameOpened{
        ...
        require(msg.value == 0, "only TBS");
        require(msg.tokenid == tokenIdTBS, "TBS only");
        require(msg.tokenvalue >= 20e6 && msg.tokenvalue < address(_CoinPool).tokenBalance(tokenIdTBS)/10, "bet tbs check");
    ...
    function openIbet(BetStruct storage ibet, uint256 betNumber, uint256 openNumber) internal returns(uint256,uint256)  {
            (address player, uint256 trxvalue, uint256 tbsvalue, uint256 number, uint32 betType) = decode(ibet.betInfoEn);
            ...
            if(trxvalue > 0){
                ...
                totalValue = isWin(betType, openNumber, tbsvalue);
                dealTBS(player, tbsvalue, totalValue);
            }
    ...
    function xopen(uint256 num) public view returns(bytes32 id, bytes32 hashbyte, uint256 number, uint256 openNumber, address player, uint32 betType, uint256 trxvalue, uint256 tbsvalue, uint256 winvalue){
        return xopenWithHash(num, 0);
    }

    function xopenWithHash(uint256 num, uint256 hashnum) public view returns(bytes32 id, bytes32 hashbyte, uint256 number, uint256 openNumber, address player, uint32 betType, uint256 trxvalue, uint256 tbsvalue, uint256 winvalue){
        BetStruct storage ibet = BetRecordExtend[num];
        return preopenWithHash(uint256(ibet.betInfoEn), hashnum);
    }

    function preopenWithHash(uint256 id, uint256 hashnum) public view  returns(bytes32 rid, bytes32 hashbyte, uint256 number, uint256 openNumber, address player, uint32 betType, uint256 trxvalue, uint256 tbsvalue, uint256 winvalue){
        (player, trxvalue, tbsvalue, number, betType) = decode(bytes32(id));
        if(hashnum > 0)
            hashbyte = bytes32(hashnum);
        if(uint256(hashbyte) == 0)
            hashbyte = getHashByNumberUnsafe(number);
        openNumber = hashNumber(hashbyte);
        winvalue = isWin(betType, openNumber, trxvalue>0?trxvalue:tbsvalue);
        rid = bytes32(id);
    }

    function preopen(uint256 id) public view  returns(bytes32 rid, bytes32 hashbyte, uint256 number, uint256 openNumber, address player, uint32 betType, uint256 trxvalue, uint256 tbsvalue, uint256 winvalue){
        return preopenWithHash(id, 0);
    }

    function getMyRecord() public view returns(bytes32 id, bytes32 _hash, uint256 rtx, uint256 tbs){
        if (BetRecordExtend.length > 0){
            ...
            for(uint256 i = BetRecordExtend.length; i > limit; i--){
                ...
                (address player, uint256 trxvalue, uint256 tbsvalue, uint256 number,uint32 betType) = decode(bid);
                trxvalue;tbsvalue;betType;
                if (player == msg.sender){
                    return (bid, getHashByNumberUnsafe(number), msg.sender.balance, msg.sender.tokenBalance(tokenIdTBS));
                }
            }
        }
        return (0,0, msg.sender.balance, msg.sender.tokenBalance(tokenIdTBS));
    }

    function getMyBalance() public view returns(uint256 rtx, uint256 tbs) {
        return (msg.sender.balance, msg.sender.tokenBalance(tokenIdTBS));
    }
